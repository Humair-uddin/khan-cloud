...
#define PIPE_NAME L"\\\\.\\pipe\\KhanCloudVirtualDisplay"
...
wstring gpuname;
wstring confpath = L"C:\\ProgramData\\KhanCloud\\VirtualDisplay";
...
bool EnabledQuery(const std::wstring& settingKey) {
	...
	std::wstring settingsname = confpath + L"\\khan-vdd-settings.xml";
	HKEY hKey;
	DWORD dwValue;
	DWORD dwBufferSize = sizeof(dwValue);
	LONG lResult = RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"SOFTWARE\\KhanCloud\\VirtualDisplay", 0, KEY_READ, &hKey);
...
/*
 * KG-008K7 — IddCx/Windows HDR runtime state.
 *
 * g_HdrMetadataStore remains Khan capability/policy state.
 * This structure tracks state supplied or selected by Windows.
 */
struct KhanHdrRuntimeState
{
        IDDCX_HDR10_METADATA DefaultMetadata = {};
        bool HasDefaultMetadata = false;

        IDDCX_HDR10_METADATA CurrentFrameMetadata = {};
        bool HasCurrentFrameMetadata = false;

        IDDCX_WIRE_FORMAT_INFO CommittedWireFormat = {};
        bool HasCommittedWireFormat = false;

        bool PathActive = false;
};

static std::map<IDDCX_MONITOR, KhanHdrRuntimeState>
        g_KhanHdrRuntimeState;

static std::mutex g_KhanHdrRuntimeMutex;
...
VddHdrMetadata ConvertEdidToSmpteMetadata(const EdidProfileData& profile) {
    ...
    // Use configured content light levels (from khan-vdd-settings.xml)
...
bool UpdateXmlToggleSetting(bool toggle, const wchar_t* variable) {
	const wstring settingsname = confpath + L"\\khan-vdd-settings.xml";
	CComPtr<IStream> pFileStream;
...
bool UpdateXmlGpuSetting(const wchar_t* gpuName) {
	const std::wstring settingsname = confpath + L"\\khan-vdd-settings.xml";
	CComPtr<IStream> pFileStream;
	HRESULT hr = SHCreateStreamOnFileEx(settingsname.c_str(), STGM_READWRITE, FILE_ATTRIBUTE_NORMAL, FALSE, nullptr, &pFileStream);
	if (FAILED(hr)) {
		vddlog("e", "UpdatingXML: XML file could not be opened.");
...
bool initpath() {
	...
	LONG lResult;
	//vddlog("i", "Reading reg: Computer\\HKEY_LOCAL_MACHINE\\SOFTWARE\\KhanCloud\\VirtualDisplay");           Remove this due to the fact, if reg key exists, this is called before reading
	lResult = RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"SOFTWARE\\KhanCloud\\VirtualDisplay", 0, KEY_READ, &hKey);
	if (lResult != ERROR_SUCCESS) {
		ostringstream oss;
...
void loadSettings() {
	...
	if (PathFileExistsW(filename.c_str())) {
		...
		*/
		...
		vddlog("i","Using khan-vdd-settings.xml");
		return;
	}
...
NTSTATUS VirtualDisplayDriverDeviceAdd(WDFDRIVER Driver, PWDFDEVICE_INIT pDeviceInit)
{
	...
	if (gpuname.empty() || gpuname == L"default") {
		...
		logStream << "Loading GPU from khan-vdd-settings.xml";
	}
...
HRESULT Direct3DDevice::Init()
{
	...
	{
		FILE* file;
		fopen_s(&file, "C:\\ProgramData\\KhanCloud\\VirtualDisplay\\desc_hdr.bin", "wb");
	...
	for (;;)
	{
		...
		if (IDD_IS_FUNCTION_AVAILABLE(
                        IddCxSwapChainReleaseAndAcquireBuffer2))
                {
                        ...
                        {
                                pSurface =
                                    Buffer.MetaData.pSurface;

                                const UINT validFlags =
                                    static_cast<UINT>(
                                        Buffer.MetaData.ValidFlags
                                    );
                                ...
                                )
                                {
                                        ...
                                        case IDDCX_HDR10_FRAME_METADATA_TYPE_DEFAULT:
                                                if (
                                                    state.HasDefaultMetadata
                                                )
                                                {
                                                        state.CurrentFrameMetadata =
                                                            state.DefaultMetadata;

                                                        state.HasCurrentFrameMetadata =
                                                            true;
                                                }
                                                else
                                                {
                                                        ...
                                                        vddlog(
                                                            "w",
                                                            "HDR DEFAULT requested before default metadata was supplied"
                                                        );
                                                }
                                                break;

                                        case IDDCX_HDR10_FRAME_METADATA_TYPE_UNCHANGED:
                                                // Preserve previous frame metadata.
                                                break;
                                        ...
                                        case IDDCX_HDR10_FRAME_METADATA_TYPE_UNINITIALIZED:
                                        default:
                                                state.HasCurrentFrameMetadata =
                                                    false;
                                                break;
...
void IndirectDeviceContext::InitAdapter()
{
	...
	AdapterCaps.EndPointDiagnostics.pEndPointFriendlyName = L"Khan Virtual Display";
	AdapterCaps.EndPointDiagnostics.pEndPointManufacturerName = L"Khan Cloud";
	AdapterCaps.EndPointDiagnostics.pEndPointModelName = L"Khan Cloud Virtual Monitor";
...
void IndirectDeviceContext::CreateMonitor(unsigned int index) {
	...
	MonitorInfo.MonitorDescription.pData = IndirectDeviceContext::s_KnownMonitorEdid.data();
        ...
        /*
         * KG-008K8 temporary diagnostic:
         * Capture the exact EDID buffer immediately before
         * IddCxMonitorCreate consumes MonitorInfo.
         *
         * This is intentionally at the IddCx boundary so that
         * configuration, loadEdid(), maincalc(), and any earlier
         * transformations are already complete.
         */
        {
                ...
                CreateDirectoryW(
                        L"C:\\ProgramData\\KhanCloud",
                        nullptr
                );

                CreateDirectoryW(
                        diagnosticDirectory.c_str(),
                        nullptr
                );

                const std::wstring binaryPath =
                        diagnosticDirectory +
                        L"\\kg008k8-runtime-edid.bin";
                ...
                const std::wstring textPath =
                        diagnosticDirectory +
                        L"\\kg008k8-runtime-edid.txt";
                ...
                if (
                    ...
                    diagnosticFile != nullptr
                )
                {
                        fwprintf(
                            diagnosticFile,
                            L"KG008K8_IDDCX_BOUNDARY=YES\n"
                        );
                        ...
                        fwprintf(
                            diagnosticFile,
                            L"CUSTOM_EDID=%ls\n",
                            customEdid ? L"true" : L"false"
                        );

                        fwprintf(
                            diagnosticFile,
                            L"PREVENT_SPOOF=%ls\n",
                            preventManufacturerSpoof
                                ? L"true"
                                : L"false"
                        );
                        ...
                        )
                        {
                                fwprintf(
                                    diagnosticFile,
                                    L"%02X%s",
                                    runtimeEdid[byteIndex],
                                    (
                                        byteIndex + 1 == dumpCount
                                    ) ? L"" : L" "
                                );
                        }
...
void IndirectDeviceContext::AssignSwapChain(IDDCX_MONITOR Monitor, IDDCX_SWAPCHAIN SwapChain, LUID RenderAdapter, HANDLE NewFrameEvent)
{
	...
	else
	{
		...
		auto newProcessor = std::make_unique<SwapChainProcessor>(
                    Monitor,
                    SwapChain,
                    Device,
                    NewFrameEvent
...
)
{
        ...
        /*
         * Khan advertises native 8/10-bpc target modes.
         * DitheringSupport means actual precision-reduction
         * capability; it is not a BPC support mask.
         */
        pOutArgs->DitheringSupport = {};

        stringstream logStream;
        logStream
                << "IddCx target capability query:"
                << "\n  ConnectorIndex: "
                << pInArgs->ConnectorIndex
                << "\n  WideColorSpace: Yes"
                << "\n  HighColorSpace: Yes"
                << "\n  Dithering: Not advertised";
...
{
        if (MonitorObject == nullptr || pInArgs == nullptr)
        {
                return STATUS_INVALID_PARAMETER;
        }

        if (pInArgs->Type != IDDCX_HDRMETADATA_TYPE_HDR10)
        {
                vddlog(
                    "w",
                    "Unsupported Windows default HDR metadata type"
                );
        ...
        if (
            pInArgs->Data.pHdr10 == nullptr ||
            pInArgs->Size < sizeof(IDDCX_HDR10_METADATA)
        )
        {
                vddlog(
                    "e",
                    "Invalid Windows HDR10 default metadata"
                );
...

        {
                ...
                auto& state =
                    g_KhanHdrRuntimeState[MonitorObject];
        ...
        vddlog(
            "i",
            "Saved Windows-provided default HDR10 metadata"
        );
...
)
{
        ...
        for (
            UINT index = 0;
            index < pInArgs->PathCount;
            ++index
        )
        {
                const IDDCX_PATH2& path =
                    pInArgs->pPaths[index];
...

                {
                        ...
                        state.PathActive = active;

                        if (active)
                        {
                                state.CommittedWireFormat =
                                    path.WireFormatInfo;

                                state.HasCommittedWireFormat =
                                    true;
                        }
                        else
                        {
                ...
                stringstream logStream;
                logStream
                        << "Committed IddCx 1.10 path:"
                        << "\n  Index: " << index
                        << "\n  Active: "
                        << (active ? "Yes" : "No")
                        << "\n  ColorSpace: "
                        << static_cast<UINT>(
                            path.WireFormatInfo.ColorSpace
                        )
                        << "\n  RGB BPC: "
                        << static_cast<UINT>(
                            path.WireFormatInfo
                                .BitsPerComponent.Rgb
                        );
