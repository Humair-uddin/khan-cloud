    ...
    *,
    ...
    with tempfile.TemporaryDirectory(
        ...
        install_ps1.write_text(
            ...
            '$Bootstrap = "$Here\\agent\\deploy\\universal-bootstrap.ps1"\n'
            '$Scripts = @(\n'
            '    $Bootstrap,\n'
            '    "$Here\\agent\\deploy\\install-runtime.ps1",\n'
            '    "$Here\\agent\\deploy\\apply-runtime-update.ps1"\n'
            ')\n'
            'foreach ($Script in $Scripts) {\n'
            '    if (-not (Test-Path $Script -PathType Leaf)) {\n'
            '        throw "Required Windows installer script is missing: $Script"\n'
            '    }\n'
            '    $Tokens = $null\n'
            '    $Errors = $null\n'
