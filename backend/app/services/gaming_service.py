...
def select_gaming_host(db: Session, *, minimum_vram_mb: int, cpu: int, memory_bytes: int, storage_bytes: int):
    rows = db.execute(
...
def create_gaming_session(db: Session, *, payload: GamingSessionCreate, actor: User) -> GamingSession:
    ...
    capacity, node, gpu_uuid, gpu_name, gpu_vram = select_gaming_host(
        db, minimum_vram_mb=payload.minimum_vram_mb, cpu=payload.cpu,
    ...
    session = GamingSession(
        ...
        minimum_vram_mb=payload.minimum_vram_mb, requested_cpu=payload.cpu,
    ...
    db.add(NodeJob(
        ...
        payload={"session_id": str(session.id), "gpu_uuid": gpu_uuid, "gpu_name": gpu_name,
                 "minimum_vram_mb": payload.minimum_vram_mb, "streaming_backend": payload.streaming_backend},
    ))
