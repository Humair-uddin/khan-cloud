...
class GamingSession(BaseModel):
    ...
    quarantine_recovery_state: Mapped[str] = mapped_column(String(30), nullable=False, default="", index=True)
    quarantine_recovery_attempt_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    quarantine_recovery_requested_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    quarantine_recovery_last_attempt_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    quarantine_recovery_last_source: Mapped[str] = mapped_column(String(30), nullable=False, default="")
    quarantine_recovery_last_message: Mapped[str] = mapped_column(String(500), nullable=False, default="")
    quarantine_recovery_last_evidence: Mapped[dict[str, Any]] = mapped_column(JSONB, default=dict)
    quarantine_recovered_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
